...
void OLED_ClearChar(u8 x, u8 y)//清空某段字符
{
  u8 i = 0;
	OLED_Set_Pos(x, y);
	for(i = 0; i < 16; i++)
...
void OLED_ShowChar24(u8 x, u8 y, u8 chr)
{
  u8 c=0, i = 0;
  c = chr - '0'; //得到偏移后的值
  if(x > 130 - 1)
  {
	...
	OLED_Set_Pos(x, y);
	for(i = 0; i < 12; i++)
		Write_IIC_Data(zf24[c * 36 + i]);
	OLED_Set_Pos(x, y + 1);
	for(i = 0; i < 12; i++)
		Write_IIC_Data(zf24[c * 36 + i + 12]);
	OLED_Set_Pos(x, y + 2);
	for(i = 0; i < 12; i++)
		Write_IIC_Data(zf24[c * 36 + i + 24]);

}
void OLED_ClearChar24(u8 x, u8 y)
{
  u8 i = 0;
	OLED_Set_Pos(x, y);
	for(i = 0; i < 24; i++)
